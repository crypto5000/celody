...
/* "downhill" module requires 1 instrument (primary-secondary pair) only - no multiple instruments
*  randomly changes to always go lower pitch and slower tempo
...
var celodyLoopTimer;
celody.downHill = 1;
function playDownHillStream(celody,loopCount) {                                                     
    celody.countIndex = 0;
    celody.countMax = 0;
    ...
    if (loopCount === 0) {
        //reset the current downhill rate
        celody.downHill = celody.playRate;                
    }
    ...
    while (celody.countIndex < celody.countMax) {      
        ...
        // adjust the new soundrate for the downhill effect
        if (Math.random() < (celody.streamInfo[0].variety * 0.5)) {
            if (loopCount === 0) {
                celody.downHill = celody.playRate * 0.75;
            } else if (loopCount === 1) {
                celody.downHill = celody.playRate * 0.66;
            } else if (loopCount > 1) {
                celody.downHill = celody.playRate * 0.5;
            }        
        } 
        celody.soundTiming[celody.countIndex].newSoundRate = celody.downHill;
        // set a floor for the downhill
    ...
    if (celody.streamInfo[0].stop) {
        ...
        playDownHillStream(celody,loopCount);
    } else {                                  
        celodyLoopTimer = setTimeout(function(){ playDownHillStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
    }
