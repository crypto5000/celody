...
/* "location" module requires user permission to access geolocation/gps API for lat-lon
*  tempo/pitch changes as person moves from starting latitude-longitude position
                ...
                celody.soundFile[celody.counter] = new Pizzicato.Sound({ 
                    ...
                    if (error) {                                
                        ...
                        if (celody.soundLoaded == celody.streamInfo[0].totalSounds) {
                            ...
                            playLocationInitStream(celody,0);
                        }
                    }
                });                                                                          
...
var celodyLoopTimer;
// new york city lat-lon defaults
celody.latitude = 40.71455;
celody.longitude = -74.00712;
celody.startLatitude = 40.71455;
celody.startLongitude = -74.00712;
function playLocationInitStream(celody,loopCount) {     
  // get the location before the stream starts
  function successLocationInit(position) {
      celody.latitude  = position.coords.latitude;
      celody.longitude = position.coords.longitude;
      celody.startLatitude  = position.coords.latitude;
      celody.startLongitude = position.coords.longitude;
      ...
      if ((!Number.isFinite(celody.startLatitude)) || (Number(celody.startLatitude) < -90) || (Number(celody.startLatitude) > 90)) {                       
          celody.startLatitude = 40.71455;
      }
      ...
      if ((!Number.isFinite(celody.startLongitude)) || (Number(celody.startLongitude) < -180) || (Number(celody.startLongitude) > 180)) {                       
          celody.startLongitude = -74.00712;
      }
      // move on to playing stream
      playLocationStream(celody,loopCount);
  }

  function errorLocationInit() {
      // on error, default to new york city lat-lon above
      playLocationStream(celody,loopCount);
  }

  if (!navigator.geolocation) {
      // browser does not support geolocation, default to new york city
      playLocationStream(celody,loopCount);
  } else {
      navigator.geolocation.getCurrentPosition(successLocationInit, errorLocationInit);
  }

}

function playLocationStream(celody,loopCount) {
  celody.countIndex = 0;
  celody.countMax = 0;
  ...
  // update the latitude-longitude for movements in position
  ...
  function errorLocation() {
      // on error, default to current value of coordinates
  }

  // adjust the playRate for the location
  celody.playRate = celody.streamInfo[0].playSpeed;

  // on first loop
  if (loopCount === 0) {
      // set the base start playRate to 1 - overrides any file playSpeed
      celody.playRate = 1;
  } else {
      // after first loop - adjust playRate based on lat/lon offset from starting position
      celody.playRate = 1;
      // include variety parameter in adjustment
      if (Math.random() < celody.streamInfo[0].variety) {
          // 50-50 increment or decrement from the starting position
          if (Math.random() > 0.5) {
              celody.playRate = celody.playRate + (((celody.startLatitude - celody.latitude) + (celody.startLongitude - celody.longitude)) * 20);
          } else {
              celody.playRate = celody.playRate - (((celody.startLatitude - celody.latitude) + (celody.startLongitude - celody.longitude)) * 20);
          }
      }
      ...
      if (Number(celody.playRate) < 0.5) {
          celody.playRate = 0.5 + (0.5 * Math.random());
      }
      if (Number(celody.playRate) > 1.5) {
          celody.playRate = 1.5 - (0.5 * Math.random());
      }
  }
  ...
  // async timing may delay lat-lon updates - running at 50% of loop time to compensate
  if (!navigator.geolocation) {
      // browser does not support geolocation, default to new york city
  } else {
      // wait until half the loop runs before getting updated position
      setTimeout(function(){ navigator.geolocation.getCurrentPosition(successLocation, errorLocation); }, (celody.timePerLoop * 0.5 * 1000));
  }
  ...
  if (celody.streamInfo[0].stop) {
      ...
      celodyLoopTimer = setTimeout(function(){ playLocationStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
  }
  return false;
}
