...
/* "tight" module requires 1 instrument (primary-secondary pair) only - no multiple instruments
*  randomly changes tempo/pitch within narrow band
...
var celodyLoopTimer;
function playTightStream(celody,loopCount) {                                                     
    celody.countIndex = 0;
    celody.countMax = 0;
    ...
    while (celody.countIndex < celody.countMax) {      
        ...
        // adjust the new soundrate for the tight effect
        if (Math.random() < (celody.streamInfo[0].variety * 0.5)) {
            if (Math.random() > 0.5) {
                celody.soundTiming[celody.countIndex].newSoundRate = celody.playRate + (0.1 * Math.random());
            } else {
                celody.soundTiming[celody.countIndex].newSoundRate = celody.playRate - (0.1 * Math.random());
            }        
        } else {
    ...
    if (celody.streamInfo[0].stop) {
        ...
        playTightStream(celody,loopCount);
    } else {                                  
        celodyLoopTimer = setTimeout(function(){ playTightStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
    }
