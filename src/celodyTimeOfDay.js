...
/* "timeofday" module changes based on time of day, can use multiple instruments
...
var celodyLoopTimer;
function playTimeOfDayStream(celody,loopCount) {                                                     
    celody.countIndex = 0;
    celody.countMax = 0;
    ...
    celody.currentTime = celody.currentDate.getHours();  

    // adjust the playRate for the time of day
    celody.playRate = celody.streamInfo[0].playSpeed;
    if (celody.currentTime > 18) {
        celody.playRate = 1.33 * celody.playRate;
    } else if (celody.currentTime > 12) {
        celody.playRate = 1.0 * celody.playRate;
    } else if (celody.currentTime > 6) {
    ...
    if (celody.streamInfo[0].stop) {
        ...
        playTimeOfDayStream(celody,loopCount);
    } else {                                  
        celodyLoopTimer = setTimeout(function(){ playTimeOfDayStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
    }
    return false;
}
