...
/* "weekend" module changes based on day of week, can use multiple instruments
...
var celodyLoopTimer;
function playWeekendStream(celody,loopCount) {                                                     
    celody.countIndex = 0;
    celody.countMax = 0;
    ...
    // tempo shifting using variety                                       
    celody.timePerBase = ((60 / (Number(celody.streamInfo[0].tempo) + Math.floor(Math.random() * celody.streamInfo[0].tempo * celody.streamInfo[0].variety) + 1) ) * (celody.streamInfo[0].beatsPerBar / celody.streamInfo[0].baseUnit)); // seconds per base unit note
    celody.timePerLoop = celody.timePerBase * celody.streamInfo[0].baseUnit * celody.streamInfo[0].bars; // seconds per loop

    // get the current day
    celody.currentDate = new Date(); 
    celody.currentDay = celody.currentDate.getDay();  

    // adjust the playRate for the day
    celody.playRate = celody.streamInfo[0].playSpeed;
    if (celody.currentDay === 0) {
        celody.playRate = 1.33 * celody.playRate;
    } else if (celody.currentDay === 6) {
        celody.playRate = 1.25 * celody.playRate;
    } else if (celody.currentDay === 1) {
        celody.playRate = 0.75 * celody.playRate;
    } else if (celody.currentDay === 3) {
        celody.playRate = 0.66 * celody.playRate;
    } else {
        celody.playRate = 1.00 * celody.playRate;
    }
    ...
    if (celody.streamInfo[0].stop) {
        ...
        playWeekendStream(celody,loopCount);
    } else {                                  
        celodyLoopTimer = setTimeout(function(){ playWeekendStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
    }
    return false;
}
