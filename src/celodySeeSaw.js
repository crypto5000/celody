...
/* "seesaw" module requires 1 instrument (primary-secondary pair) only - no multiple instruments
*  randomly changes to always go down (slower tempo/lower pitch) than up (faster tempo/higher pitch)
...
var celodyLoopTimer;
celody.seeSaw = 1;
function playSeeSawStream(celody,loopCount) {                                                     
    celody.countIndex = 0;
    celody.countMax = 0;
    ...
    // handle the buttons on the initial play of stream
    if (loopCount === 0) {
        //reset the current seesaw rate
        celody.seeSaw = celody.playRate;                
    }
    ...
    while (celody.countIndex < celody.countMax) {      
        ...
        // adjust the new soundrate for the seesaw effect
        if (Math.random() < (celody.streamInfo[0].variety * 0.5)) {
            if (loopCount % 5 === 0) {
                celody.seeSaw = celody.playRate * 0.75;
            } else if (loopCount % 5 === 1) {
                celody.seeSaw = celody.playRate * 1.00;
            } else if (loopCount % 5 === 2) {
                celody.seeSaw = celody.playRate * 1.25;
            } else if (loopCount % 5 === 3) {
                celody.seeSaw = celody.playRate * 1.00;
            } else if (loopCount % 5 === 4) {
                celody.seeSaw = celody.playRate * 0.66;
            }                         
        } 
        celody.soundTiming[celody.countIndex].newSoundRate = celody.seeSaw;
        // set a ceiling for the seeSaw
    ...
    if (celody.streamInfo[0].stop) {
        ...
        playSeeSawStream(celody,loopCount);
    } else {                                  
        celodyLoopTimer = setTimeout(function(){ playSeeSawStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
    }
