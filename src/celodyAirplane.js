...
/* "airplane" module requires user permission to geolocation/gps API for altitude
*  tempo/pitch changes based on user's altitude - higher altitude equals higher pitch
                ...
                celody.soundFile[celody.counter] = new Pizzicato.Sound({ 
                    ...
                    if (error) {                                
                        ...
                        if (celody.soundLoaded == celody.streamInfo[0].totalSounds) {
                            ...
                            playAirplaneInitStream(celody,0);
                        }
                    }
                });                                                                          
...
var celodyLoopTimer;
// set default altitude to sea level
celody.altitude = 0;
function playAirplaneInitStream(celody,loopCount) {
  // get the altitude before the stream starts
  function successAirplaneInit(position) {
      celody.altitude  = position.coords.altitude;
      // validate altitude
      if ((!Number.isFinite(celody.altitude)) || (Number(celody.altitude) < -10000) || (Number(celody.altitude) > 20000)) {                       
          celody.altitude = 0;
      }         
      // move on to playing stream
      playAirplaneStream(celody,loopCount);
  }

  function errorAirplaneInit() {
      // on error, default to sea level
      playAirplaneStream(celody,loopCount);
  }
  ...
  if (!navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(successAirplaneInit, errorAirplaneInit);
  }

}

function playAirplaneStream(celody,loopCount) {
  celody.countIndex = 0;
  celody.countMax = 0;
  ...
  // update the altitude
  function successAirplane(position) {
      celody.altitude  = position.coords.altitude;
      // validate altitude
      if ((!Number.isFinite(celody.altitude)) || (Number(celody.altitude) < -10000) || (Number(celody.altitude) > 20000)) {                       
          celody.altitude = 0;
  ...
  function errorAirplane() {
      // on error, default to current level
  }

  // adjust the playRate for the altitude in meters relative to sea level
  celody.playRate = celody.streamInfo[0].playSpeed;
  if (Number(celody.altitude) <= 0) {
      celody.playRate = 0.5 + (0.1 * Math.random());
  } else if (Number(celody.altitude) < 100) {
      celody.playRate = 0.66 + ((0.75 - 0.66) * (Number(celody.altitude)/100));
  } else if (Number(celody.altitude) < 1000) {
      celody.playRate = 0.75 + ((1.0 - 0.75) * (Number(celody.altitude)/1000));
  } else if (Number(celody.altitude) < 10000) {
      celody.playRate = 1.0 + ((1.25 - 1.0) * (Number(celody.altitude)/10000));
  } else if (Number(celody.altitude) < 30000) {
      celody.playRate = 1.25 + ((1.33 - 1.25) * (Number(celody.altitude)/30000));
  } else if (Number(celody.altitude) < 50000) {
      celody.playRate = 1.33 + ((1.5 - 1.33) * (Number(celody.altitude)/50000));
  } else {
  ...
  if (!navigator.geolocation) {
      ...
      setTimeout(function(){ navigator.geolocation.getCurrentPosition(successAirplane, errorAirplane); }, (celody.timePerLoop * 0.5 * 1000));
  }
  ...
  if (celody.streamInfo[0].stop) {
      ...
      celodyLoopTimer = setTimeout(function(){ playAirplaneStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
  }
  return false;
}
