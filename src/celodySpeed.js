...
/* "speed" module requires user permission to geolocation/gps API for lat-lon and speed
*  tempo/pitch changes faster for faster speeds and slower for slower speeds
...
// set default speed
celody.speed = 0;
function playSpeedInitStream(celody,loopCount) {
  // get the speed before the stream starts
  function successSpeedInit(position) {
      celody.speed  = position.coords.speed;
      // validate speed
      if (!celody.speed) {
          // speed not returned by device, calculate using lat-lon
          ...
          if (celody.latLonFlag) {
              ...
              setTimeout(function(){ navigator.geolocation.getCurrentPosition(successSpeedInitCalc, errorSpeedInit); }, (1.9 * 1000));     
          } else {
              // no lat-lon to calculate speed, default to zero
              celody.speed = 0;
              // move on to playing stream
              playSpeedStream(celody,loopCount);
          }
      } else if ((!Number.isFinite(celody.speed)) || (Number(celody.speed) < 0) || (Number(celody.speed) > 500)) {                       
          celody.speed = 0;
          // move on to playing stream
          playSpeedStream(celody,loopCount);         
      } else {
  ...
  function successSpeedInitCalc(position) {
      ...
      if (celody.latLonFlag) {
          // calculate the speed using 1.9 second offset from current lat-lon position
          calculateSpeed(celody,loopCount,celody.latitude,celody.longitude,position.coords.latitude,position.coords.longitude,1.9*1000);     
      } else {
          // no lat-lon to calculate speed, default to zero
          celody.speed = 0;
          // move on to playing stream
  ...
  function errorSpeedInit() {
      // on error, default to zero speed
      playSpeedStream(celody,loopCount);
  }
  ...
  if (!navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(successSpeedInit, errorSpeedInit);
  }

}

function calculateSpeed(celody,loopCount,latitude1,longitude1,latitude2,longitude2,timeDelta) {
  ...
  const r = 6378100;
  var rho1 = r * Math.cos(latitude1);
  var z1 = r * Math.sin(latitude1);
  var x1 = rho1 * Math.cos(longitude1);
  var y1 = rho1 * Math.sin(longitude1);
  var rho2 = r * Math.cos(latitude2);
  var z2 = r * Math.sin(latitude2);
  var x2 = rho2 * Math.cos(longitude2);
  var y2 = rho2 * Math.sin(longitude2);
  var dot = (x1 * x2 + y1 * y2 + z1 * z2);
  var cos_theta = dot / (r * r);
  var theta = Math.acos(cos_theta);
  var distanceMeters = r * theta;
  // if distance is less than 1.5 meters, set to zero due to gps accuracy
  if (distanceMeters < 1.5) {distanceMeters = 0;}        
  if (timeDelta > 0) {
      celody.speed = distanceMeters / (timeDelta/1000);
  } else {
      celody.speed = 0;
  }
  // validate speed
  if ((!Number.isFinite(celody.speed)) || (Number(celody.speed) < 0) || (Number(celody.speed) > 500)) {                       
      celody.speed = 0;
...
function playSpeedStream(celody,loopCount) {
  celody.countIndex = 0;
  celody.countMax = 0;
  ...
  function successSpeed(position) {
      celody.speed  = position.coords.speed;
      // validate speed
      if (!celody.speed) {
          // speed not returned by device, calculate using lat-lon
          ...
          if (celody.latLonFlag) {
              // calculate the speed using 1.9 second offset from current lat-lon position
              setTimeout(function(){ navigator.geolocation.getCurrentPosition(successSpeedCalc, errorSpeed); }, (1.9 * 1000));     
          } else {
              // no lat-lon to calculate speed, default to zero
              celody.speed = 0;
  ...
  function successSpeedCalc(position) {
      // get the second lat-lon to calculate speed manually   
      celody.latLonFlag = true;
      // validate lat should be between -90 and 90
      if ((!Number.isFinite(position.coords.latitude)) || (Number(position.coords.latitude) < -90) || (Number(position.coords.latitude) > 90)) {                       
  ...
  // adjust the playRate for the speed in meters/sec
  celody.playRate = celody.streamInfo[0].playSpeed;
  if (Number(celody.speed) <= 0) {
      celody.playRate = 0.5 + (0.1 * Math.random());
  } else if (Number(celody.speed) < 2) {
      celody.playRate = 0.66 + ((0.75 - 0.66) * (Number(celody.speed)/2));
  } else if (Number(celody.speed) < 5) {
      celody.playRate = 0.75 + ((1.0 - 0.75) * (Number(celody.speed)/5));
  } else if (Number(celody.speed) < 15) {
      celody.playRate = 1.0 + ((1.25 - 1.0) * (Number(celody.speed)/15));
  } else if (Number(celody.speed) < 25) {
      celody.playRate = 1.25 + ((1.33 - 1.25) * (Number(celody.speed)/25));
  } else if (Number(celody.speed) < 35) {
      celody.playRate = 1.33 + ((1.5 - 1.33) * (Number(celody.speed)/35));
  } else {
      celody.playRate = 1.5 - (0.1 * Math.random());
  }

  // bound the playRate to prevent extremes
  if (Number(celody.playRate) < 0.5) {
      celody.playRate = 0.5;
  }

  if (Number(celody.playRate) > 1.5) {
      celody.playRate = 1.5;
  }
  ...
  if (!navigator.geolocation) {
      ...
      setTimeout(function(){ navigator.geolocation.getCurrentPosition(successSpeed, errorSpeed); }, (celody.timePerLoop * 0.5 * 1000));
  }
  ...
  if (celody.streamInfo[0].stop) {
      ...
      celodyLoopTimer = setTimeout(function(){ playSpeedStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
  }
  return false;
}
