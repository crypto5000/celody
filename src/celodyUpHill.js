...
/* "uphill" module requires 1 instrument (primary-secondary pair) only - no multiple instruments
*  randomly changes to always go higher pitch and faster tempo
...
var celodyLoopTimer;
celody.upHill = 1;
function playUpHillStream(celody,loopCount) {                                                     
    celody.countIndex = 0;
    celody.countMax = 0;
    ...
    if (loopCount === 0) {
        //reset the current uphill rate
        celody.upHill = celody.playRate;                
    }
    ...
    while (celody.countIndex < celody.countMax) {      
        ...
        // adjust the new soundrate for the uphill effect
        if (Math.random() < (celody.streamInfo[0].variety * 0.5)) {
            if (loopCount === 0) {
                celody.upHill = celody.playRate * 1.25;
            } else if (loopCount === 1) {
                celody.upHill = celody.playRate * 1.33;
            } else if (loopCount === 2) {
                celody.upHill = celody.playRate * 1.5;
            } else if (loopCount > 2) {
                celody.upHill = celody.playRate * 1.75;
            }        
        } 
        celody.soundTiming[celody.countIndex].newSoundRate = celody.upHill;
        // set a ceiling for the uphill
    ...
    if (celody.streamInfo[0].stop) {
        ...
        playUpHillStream(celody,loopCount);
    } else {                                  
        celodyLoopTimer = setTimeout(function(){ playUpHillStream(celody,loopCount); }, (celody.timePerLoop * 1000) - (performance.now() - celody.masterStart));                                       
    }
